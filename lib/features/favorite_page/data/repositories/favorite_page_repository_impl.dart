...
class FavoritePageRepositoryImpl implements FavoritePageRepository {
  ...
  @override
  Future<({Failure? error, List<PostModel>? post})> deleteAddOrGetLocalPosts(
      {PostModel? post, required PostAction action}) async {
    try {
      final result = await localDataSource.deleteAddOrGetLocalPosts(
      ...
      switch (result) {
        case var list?:
          return (
            error: null,
            post: list
          ); // if List<PostModel> is NOT null and there`s no any exceptions
        default:
          return (
            error: null,
            post: null
          ); // if insert/delete operations were success and there`s no any exceptions
      }
    } catch (_) {
      // capture rethrow from localDataSource
